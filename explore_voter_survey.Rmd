...
### *How I Discovered My Cherished Beliefs Were Totally Wrong*
...
I read the [article by Lee Drutman](https://www.voterstudygroup.org/publications/2016-elections/political-divisions-in-2016-and-beyond) referenced by The Times and was discouraged to see little support for libertarian positioning in the 2016 survey of voters by [](YouGov.com).  Fortunately, the raw data is all available and provides a rich trove for exploration.  This survey was updated in December 2018 and the data can be found [here.](https://www.voterstudygroup.org/publications/2018-voter-survey/2018-voter-survey-top-lines).   This will be the starting point for our analysis.  The survey results are a CSV file in a zipped package.  The file is in my Github repo but, as a matter of courtesy, please request it from The Voter Study Group, as their usage terms stipulate.

As always, we will be working in the **Tidyverse** dialect.  Start by loading in the raw data. As a matter of style, I like to keep a raw data frame in pristine form and manipulate a copy so if I mess up I can always restart the data munging from the same base.  If memory allows, additional intermediate versions might be helpful.
...
Now we massage the raw data a few ways.  First we `gather()` the data to group the interesting demographic features as separate variables and tidy up all the questions and answers into two variables.
...
The final step in massaging the data is to rescale all the question answers to between one and minus one, interpreted as liberal to conservative, respectively, in two dimensions. "Don't know" (usually coded as 8) is treated as neutral (zero).  "Decline to answer" (coded as 97) is set to `NA`.   If the question is "fiscal", set "social" to `NA` and vice versa.
```{r}
...
The first thing to note is the values are all over the chart.  There is not much correlation between the axes.  A linear regression shows no statistical significance with a p-value of zero but visually you can see the relationship.  Additionally, you can see that the center of the distribution lies a bit to the north of zero, socially liberal.  Has the country moved more in this direction over the years?  

As a side note, we've added some random "jitter" noise to the position of each point with `geom_jitter()`.  Otherwise, many of the points would overlap and obscure the density of the points.
```{r}
lm(scores$social~scores$fiscal) %>% broom::tidy()
```

Sadly, for a libertarian, the scores tend to slightly line up along the 45 degree axis, which means people who are more socially conservative are more likely to be fiscally conservative as well.  The libertarian quadrant is the upper left, which is quite sparseley populated.  Let's count them.
